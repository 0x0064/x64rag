...
A walkthrough showing how to use the `x64rag` CLI to analyze customer support tickets, classify them by type, check agent responses against company policy, analyze multi-turn conversations, and evaluate response quality.
...
```bash
x64rag reasoning analyze --file ticket-01-wrong-size.md --summarize --dimensions dimensions.json
```
...
```bash
x64rag reasoning analyze --file ticket-02-subscription-cancel.md --summarize --dimensions dimensions.json
```
...
```bash
x64rag reasoning classify --file ticket-01-wrong-size.md --categories categories.json
```
...
```bash
x64rag reasoning classify --file ticket-03-bulk-pricing.md --categories categories.json
```
...
for ticket in ticket-*.md; do
  ...
  category=$(x64rag reasoning --json classify --file "$ticket" --categories categories.json | jq -r '.category')
  echo "$name: $category"
done
```
...
This is where x64rag becomes a QA tool. Check whether agent responses follow your company's support policy:

```bash
x64rag reasoning compliance --file ticket-01-wrong-size.md --references response-policy.md
```
...
```bash
x64rag reasoning compliance --file ticket-04-defective-product.md --references response-policy.md
```
...
```bash
x64rag reasoning compliance --file ticket-04-defective-product.md --references response-policy.md --threshold 0.7
```
...
```bash
x64rag reasoning analyze-context --file conversation-ticket-02.json --summarize --dimensions dimensions.json
```
...
This is unacceptable. I want a full refund NOW." \
  | x64rag reasoning --json analyze --dimensions dimensions.json
```
...
for ticket in ticket-*.md; do
  echo "$(basename $ticket): $(x64rag reasoning --json classify --file $ticket --categories categories.json | jq -r .category)"
done

# 2. Check every response against policy
for ticket in ticket-*.md; do
  score=$(x64rag reasoning --json compliance --file "$ticket" --references response-policy.md | jq '.score')
  echo "$(basename $ticket): compliance $score"
done

# 3. Flag high-urgency tickets that might need follow-up
for ticket in ticket-*.md; do
  urgency=$(x64rag reasoning --json analyze --file "$ticket" --dimensions dimensions.json | jq -r '.dimensions.urgency.value')
