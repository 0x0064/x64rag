...
Edit `~/.config/x64rag/config.toml` to add the retrieval sections:
...
Add the Voyage API key to `~/.config/x64rag/.env`:
...
```bash
x64rag retrieval --pretty query "What are all the major political factions and how do they evolve across the saga?" \
...
```bash
x64rag retrieval --pretty query "How does the sandweavers' role change from being harvested to being partners?" \
    -k araxis --session deep-research
```
...
x64rag retrieval retrieve "sandweaver origin system distant light catastrophe substrate failure" \
...
x64rag retrieval --pretty query "What happens at the end of book 8? What is the final state of the world?" \
...
x64rag retrieval --pretty query "What mysteries or unanswered questions remain?" \
    -k araxis --session book9-planning
```
...
```bash
x64rag retrieval --pretty query "What happens at the end of book 8? What is the final state of the world, characters, and political structures?" \
    -k araxis --session book9-planning
```
...
```bash
x64rag retrieval --pretty query "What mysteries or unanswered questions remain? The sandweavers' origin, any prophecies?" \
    -k araxis --session book9-planning
```
...
```bash
x64rag retrieval retrieve "Dhamira Keeper abilities Resonance teaching philosophy caves" \
    -k araxis --min-score 0.3
```
...
1. Retrieve specific lore for each scene (`x64rag retrieval retrieve "..." -k araxis`)
2. Write the chapter grounded in those details
3. Use `x64rag retrieval query` to ask clarifying questions mid-draft
...
```bash
x64rag reasoning compliance --file draft-book-09-chapter-02.md \
  --references scifi-library/book-01-sands-of-araxis.md \
...
```bash
x64rag retrieval ingest draft-book-09-chapter-02.md -k araxis --source-type draft
```
...
```bash
x64rag retrieval --pretty query "The draft introduces 'makers' of the sandweavers. Does this contradict anything established about sandweaver origins?" \
    -k araxis --session book9-planning
```
...
```bash
x64rag reasoning classify --file draft-book-09-chapter-02.md --categories categories.json
```
...
```bash
x64rag reasoning analyze --file draft-book-09-chapter-02.md --summarize \
  --dimensions dimensions.json
```
...
```bash
x64rag reasoning evaluate --generated draft-book-09-chapter-02.md \
...
```bash
x64rag retrieval knowledge remove 19ad92b2-c119-4945-a458-a0ee621c64ec
```
