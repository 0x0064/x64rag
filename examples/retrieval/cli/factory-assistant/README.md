...
```bash
$ for f in factory-assistant/manual-*.md; do x64rag retrieval ingest "$f" -k factory --source-type manual; done
...
```bash
$ x64rag retrieval retrieve "compressor won't build pressure" -k factory
{
  "chunks": [
    {
...
```bash
$ x64rag retrieval retrieve "compressor won't build pressure" -k factory --min-score 0.5
{
  "chunks": [
    {
...
```bash
$ x64rag retrieval --pretty query "The heat sealer is making wrinkled seals on our PP bags. What should we check?" -k factory --min-score 0.4
...
```bash
$ x64rag retrieval --pretty query "stretch wrapper film breaks during wrapping cycle" \
...
```bash
$ x64rag retrieval --pretty query "compressor won't build pressure, what should I check first?" \
...
```bash
$ x64rag retrieval --pretty query "I checked and the reed valves look carboned up. What oil should I be using to prevent this?" \
    -k factory --session troubleshoot
...
$ x64rag retrieval session clear troubleshoot
...
```bash
$ x64rag retrieval query "What is the recommended paint color for the factory floor?" -k factory
{
...
```bash
$ x64rag retrieval --pretty query "What part numbers do I need for a 1000-hour service on the XR-500 compressor?" \
...
```bash
$ x64rag retrieval retrieve "palletizer vacuum head drops cases" -k factory --min-score 0.4
{
  "chunks": [
    {
...
```bash
$ x64rag retrieval ingest --text "2024-03-15: Replaced reed valves (RV-2201) on XR-500 unit #7. Previous valves showed excessive carbon deposits after 2,100 hours. Root cause: operator was using automotive 10W-30 oil instead of SAE 30 non-detergent. Switched to correct oil type." -k factory --source-type field-note
{
  "source_id": "a1b2c3d4-...",
  "chunk_count": 1,
  "status": "completed"
...
```bash
$ x64rag retrieval retrieve "reed valve carbon buildup cause" -k factory
```
...
```bash
# List all sources
$ x64rag retrieval knowledge list -k factory

# Get details for a specific manual
$ x64rag retrieval knowledge get 71657b57-b626-4813-ae79-e62bf42ce8bf

# Inspect how a manual was chunked
$ x64rag retrieval --pretty knowledge chunks 71657b57-b626-4813-ae79-e62bf42ce8bf

# Remove and re-ingest an updated manual
$ x64rag retrieval knowledge remove 71657b57-b626-4813-ae79-e62bf42ce8bf
$ x64rag retrieval ingest manual-01-xr500-compressor-v2.md -k factory --source-type manual
```
...
x64rag retrieval retrieve "stretch wrapper film keeps breaking" -k factory --min-score 0.4

# Multi-turn troubleshooting with session context
x64rag retrieval query "motor keeps tripping" -k factory --session ticket-4521
x64rag retrieval query "checked the thermal overload, it resets but trips again" -k factory --session ticket-4521

# An inventory system can extract part numbers from answers
x64rag retrieval query "What parts are needed for quarterly maintenance on all machines?" -k factory
