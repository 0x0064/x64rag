...
async def test_ingest_stores_content():
    ...
    await method.ingest(
        source_id="src-1", knowledge_id="kb-1", source_type="manuals",
        source_weight=1.0, title="Test Doc", full_text="Full document text here.",
        chunks=[], tags=[], metadata={},
    )
    store.store_content.assert_called_once_with(
        source_id="src-1", knowledge_id="kb-1", source_type="manuals",
        title="Test Doc", content="Full document text here.",
    )
