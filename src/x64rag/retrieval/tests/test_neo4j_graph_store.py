...
from x64rag.retrieval.stores.graph.models import GraphEntity, GraphRelation
...
async def test_compute_entity_id_deterministic():
    from x64rag.retrieval.stores.graph.neo4j import _compute_entity_id
...
async def test_validate_relation_type_known():
    from x64rag.retrieval.stores.graph.neo4j import _validate_relation_type
...
@patch("x64rag.retrieval.stores.graph.neo4j.AsyncGraphDatabase")
async def test_initialize_creates_driver_and_indexes(mock_neo4j_module, mock_driver):
    from x64rag.retrieval.stores.graph.neo4j import Neo4jGraphStore
...
async def test_add_entities_empty_list_is_noop():
    ...
    with _patch("x64rag.retrieval.stores.graph.neo4j.AsyncGraphDatabase") as mock_neo4j_module:
        driver = AsyncMock()
        session = AsyncMock()
        ctx = MagicMock()
        ctx.__aenter__ = AsyncMock(return_value=session)
