...
from x64rag.retrieval.cli.commands.session import clear_session, list_sessions, load_session, save_turn


class TestSessionStorage:
    def test_empty_session(self, tmp_path):
        with patch("x64rag.retrieval.cli.commands.session.SESSIONS_DIR", tmp_path / "sessions"):
            result = load_session("nonexistent")
        assert result == []

    def test_save_and_load(self, tmp_path):
        sessions_dir = tmp_path / "sessions"
        with patch("x64rag.retrieval.cli.commands.session.SESSIONS_DIR", sessions_dir):
