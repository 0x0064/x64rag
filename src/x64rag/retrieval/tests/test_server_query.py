...
from x64rag.retrieval.modules.generation.models import QueryResult, StreamEvent
from x64rag.retrieval.server import RagServer, RagServerConfig
...
class TestBuildRetrievalQuery:
    def test_no_history_returns_text(self):
        assert RagServer._build_retrieval_query("hello", None) == "hello"

    def test_history_appends_context(self):
        result = RagServer._build_retrieval_query("follow up", [("what is X?", "X is Y")])
        assert "follow up" in result
        assert "what is X?" in result

    def test_uses_last_3_exchanges(self):
        history = [(f"q{i}", f"a{i}") for i in range(5)]
        result = RagServer._build_retrieval_query("current", history)
        assert "q2" in result
        assert "q3" in result
        assert "q4" in result
...
class TestMergeRetrievalResults:
    def test_dedup_by_chunk_id(self):
        merged = RagServer._merge_retrieval_results(
            [_chunk("a", 0.9)],
            [_chunk("a", 0.5)],
        )
        assert len(merged) == 1
