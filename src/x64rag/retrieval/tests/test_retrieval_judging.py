...
class TestRetrievalJudgment:
    ...
    async def test_should_retrieve_true(self, judge):
        ...
        with patch("x64rag.retrieval.modules.retrieval.judging.b.JudgeRetrievalNecessity", return_value=mock_result):
    ...
    async def test_defaults_to_retrieve_on_failure(self, judge):
        with patch(
            "x64rag.retrieval.modules.retrieval.judging.b.JudgeRetrievalNecessity", side_effect=Exception("LLM error")
        ):
