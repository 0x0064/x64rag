...
def _patch_generate(answer="The MERV 13 filter is rated for fine particles."):
    return patch(
        "x64rag.retrieval.modules.generation.service.b.GenerateAnswer", new_callable=AsyncMock, return_value=answer
    )
...
def _patch_stream():
    return patch("x64rag.retrieval.modules.generation.service.b.stream.GenerateAnswer", return_value=_FakeStream())
