...
class TestStepGenerationService:
    ...
    async def test_generates_intermediate_step(self, service):
        ...
        with patch("x64rag.retrieval.modules.generation.step.b.GenerateReasoningStep", return_value=mock_result):
    ...
    async def test_baml_failure_raises_generation_error(self, service):
        with (
            patch(
                "x64rag.retrieval.modules.generation.step.b.GenerateReasoningStep", side_effect=Exception("LLM error")
            ),
    ...
    async def test_empty_chunks_uses_placeholder(self, service):
        ...
        with patch(
            "x64rag.retrieval.modules.generation.step.b.GenerateReasoningStep", return_value=mock_result
        ) as mock_call:
