...
def _patch_baml(step_returns):
    ...
    - x64rag.retrieval.baml.baml_client.async_client (the 'b' source)
    - x64rag.retrieval.baml.baml_client.types (the type classes)
    ...
    class _Ctx:
        ...
        def __enter__(self):
            ...
            p1 = patch("x64rag.retrieval.baml.baml_client.async_client.b", self.mock_b)
            # Patch the types so isinstance checks work
            p2 = patch("x64rag.retrieval.baml.baml_client.types.ToolResolvedPages", _FakeResolvedPages)
            p3 = patch("x64rag.retrieval.baml.baml_client.types.ToolFetchPages", _FakeFetchPages)
            p4 = patch("x64rag.retrieval.baml.baml_client.types.ToolDrillDown", _FakeDrillDown)
