...
from x64rag.retrieval.cli import cli, run_async
from x64rag.retrieval.cli.config import ConfigError, load_config
from x64rag.retrieval.cli.output import OutputMode, print_error, print_json, print_query_result
...
async def _query(server_config, text, knowledge_id, min_score, session_name, mode):
    from x64rag.retrieval.cli.commands.session import load_session, save_turn
    from x64rag.retrieval.server import RagServer

    history = load_session(session_name) if session_name else None

    try:
        async with RagServer(server_config) as rag:
