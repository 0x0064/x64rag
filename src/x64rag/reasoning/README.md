...
```python
from x64rag.reasoning import Message, ContextTrackingConfig
...
```python
from x64rag.reasoning import ClassificationSetDefinition
...
Install with `uv add "x64rag[cli]"`.

### Setup

```bash
x64rag reasoning init      # creates ~/.config/x64rag/config.toml + .env
x64rag reasoning status    # validates config and tests LLM connection
```
...
x64rag reasoning analyze --file input.txt --summarize
echo "some text" | x64rag reasoning analyze

# Classify text (categories from JSON file)
x64rag reasoning classify "I want my money back" --categories cats.json

# Check compliance against reference document(s)
x64rag reasoning compliance "We'll give you 150% refund" --references policy.txt
x64rag reasoning compliance "text" --references policy.md style-guide.md
x64rag reasoning compliance "text" --references reference-docs/

# Custom compliance threshold
x64rag reasoning compliance --file response.md --references policy.md --threshold 0.7

# Analyze a multi-turn conversation or document sequence
x64rag reasoning analyze-context --file conversation.json --summarize --dimensions dims.json

# Evaluate generated text against a reference
x64rag reasoning evaluate --generated output.txt --reference expected.txt --strategy judge
```
...
```bash
x64rag reasoning analyze "text"                  # pretty in terminal
x64rag reasoning analyze "text" --json           # force JSON
x64rag reasoning analyze "text" | jq .           # auto JSON when piped
```
