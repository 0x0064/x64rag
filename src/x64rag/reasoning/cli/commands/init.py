...
from x64rag.reasoning.cli import cli
from x64rag.reasoning.cli.constants import CONFIG_DIR, CONFIG_FILE, ENV_FILE
...
def init() -> None:
    ...
    if created:
        ...
        for path in created:
            click.echo(f"  {path}")
        click.echo("\nEdit these files, then run 'x64rag reasoning status' to verify.")
    else:
        click.echo("\nAlready initialized. Edit files or run 'x64rag reasoning status'.")
